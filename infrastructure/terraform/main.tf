...
resource "aws_secretsmanager_secret" "app" { name = "${var.project_name}/app" }
resource "aws_secretsmanager_secret_version" "app" {
  secret_id = aws_secretsmanager_secret.app.id
  secret_string = jsonencode({ DB_PASSWORD = var.db_password, JWT_SECRET = var.jwt_secret })
}
...
resource "aws_ecs_task_definition" "api" {
  ...
  container_definitions = jsonencode([{
    ...
    environment = [
      { name = "SPRING_DATASOURCE_URL", value = "jdbc:postgresql://${aws_db_instance.postgres.address}:5432/qa_platform?sslmode=require" },
...
resource "aws_cloudfront_origin_access_control" "frontend" {
  name                              = var.project_name
  description                       = "CloudFront to private frontend bucket"
...
resource "aws_s3_bucket_policy" "frontend" { bucket = aws_s3_bucket.frontend.id policy = data.aws_iam_policy_document.frontend_read.json }
...
output "sqs_queue_url"        { value = aws_sqs_queue.test_runs.url }
